...
if (! defined('DIRECTACESS')) exit('No direct script access allowed');
	...
	class DatabaseHandler
	{
		...
		public function __construct($host, $user, $pass, $db = '',$allow_debug=false, $extension = 'pdo')
		{       
			...
			if(in_array(strtolower($extension), $extensions)) $this->extension = strtolower($extension);
			 ...
			 if(extension_loaded('pdo') && version_compare(PHP_VERSION, '5.1.0') >= 0 && ($this->extension === '' || $this->extension === 'pdo'))
			{
				if($db === '') 
				{
			...
			else if(extension_loaded('mysqli') && function_exists('mysqli_stmt_get_result') && ($this->extension === '' || $this->extension === 'mysqli')) 
			{
		...
		public function query($sqlStatement, $keyType = "NUM", $params = array(), $paramsType = 'pdo') // $keyType = ASSOC, NUM, BOTH
		{	
